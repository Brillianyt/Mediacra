...
- config 目录有效变量数：84
- 其中 base_config 直出变量数：39
- config_meta 变量数：69
- .env.example 变量数：104

## 1) config 目录有效变量 vs config_meta

- config/*.py 消费但 config_meta 未纳入：25
  - AUTO_CLOSE_BROWSER
  ...
  - STOP_WORDS_FILE
  - USER_DATA_DIR
  - WECHAT_CSV_DEDUP_ON_FINISH
  - WECHAT_CSV_DEDUP_ON_WRITE
  - XHS_LOGIN_QRCODE_WAIT_SECONDS
  - XHS_LOGIN_REDIRECT_WAIT_SECONDS

- config_meta 有但 config/*.py 未消费：10
...
- base_config 有但 config_meta 未纳入：13
  - AUTO_CLOSE_BROWSER
  - BROWSER_LAUNCH_TIMEOUT
  - CDP_HEADLESS
  - COOKIES
...
- config_meta 有但 base_config 未定义：43
  - FEISHU_APP_ID
  - FEISHU_APP_SECRET
  - FEISHU_BATCH_SIZE
