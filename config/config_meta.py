...
CONFIG_GROUPS: List[dict] = [
    ...
    },
    {
        ...
        "icon": "sparkles",
        "fields": [
            {"key": "TEXT_AI_BASE_URL", "label": "文字 Base URL", "type": "text",
             "help": "填写兼容 OpenAI 风格的文本接口地址；Provider 与默认模型由后端自动推断"},
            {"key": "TEXT_AI_MODEL", "label": "文字模型", "type": "text",
             "help": "可选，自定义文本模型名；例如 qwen-plus、deepseek-chat"},
            {"key": "TEXT_AI_API_KEY", "label": "文字 API Key", "type": "password",
             "help": "文本模型调用使用的密钥", "sensitive": True},
            {"key": "TEXT_AI_TIMEOUT", "label": "文字超时(秒)", "type": "number",
             "help": "可选，默认沿用运行时默认超时"},
            {"key": "TEXT_AI_MAX_TOKENS", "label": "文字最大 Tokens", "type": "number",
             "help": "可选，默认沿用运行时默认上限"},
        ],
    },
    {
        ...
        "icon": "image",
        "fields": [
            {"key": "IMAGE_UNDERSTANDING_ENABLED", "label": "启用图片理解", "type": "switch",
             "help": "关闭后图片理解阶段会直接跳过"},
            {"key": "IMAGE_UNDERSTANDING_BASE_URL", "label": "图片 Base URL", "type": "text",
             "help": "填写兼容 OpenAI 风格的图片/多模态接口地址；Provider 与默认模型由后端自动推断"},
            {"key": "IMAGE_UNDERSTANDING_MODEL", "label": "图片模型", "type": "text",
             "help": "可选，自定义视觉/OCR模型名；阿里 OCR 可填 qwen-vl-ocr-latest"},
            {"key": "IMAGE_UNDERSTANDING_API_KEY", "label": "图片 API Key", "type": "password",
             "help": "图片理解调用使用的密钥", "sensitive": True},
            {"key": "IMAGE_UNDERSTANDING_MAX_IMAGES", "label": "最大图片数", "type": "number",
             "help": "单次图片理解最多送入的图片数量"},
            {"key": "IMAGE_UNDERSTANDING_TIMEOUT", "label": "图片超时(秒)", "type": "number",
             "help": "可选，默认沿用运行时默认超时"},
            {"key": "IMAGE_UNDERSTANDING_USE_LOCAL_FIRST", "label": "优先本地图片", "type": "switch",
             "help": "为后续本地图片优先识别链路预留开关"},
        ],
    },
    {
        "key": "crawler",
    ...
    },
    {
        ...
        "fields": [
            ...
            {"key": "WECHAT_APP_SECRET", "label": "微信 Secret", "type": "password", "sensitive": True, "help": "预留给微信扩展能力，当前可为空"},
            {"key": "WECHAT_CREATOR_LIST_FILE", "label": "创作者列表文件", "type": "text", "help": "JSON 文件路径，非空时优先于内置 creator 列表"},
            {"key": "WECHAT_MAX_RETRY_COUNT", "label": "最大重试次数", "type": "number", "help": "请求失败后的最大重试次数"},
            {"key": "WECHAT_RETRY_BASE_DELAY_SEC", "label": "重试基准等待(秒)", "type": "number", "help": "指数退避基准时长"},
            ...
            {"key": "WECHAT_SKIP_PAYWALL_ARTICLES", "label": "跳过付费文章", "type": "switch", "help": "开启后会跳过付费订阅文章"},
            {"key": "WECHAT_DOWNLOAD_FORMAT", "label": "下载格式", "type": "select",
             ...
             "help": "文章内容保存格式"},
            {"key": "WECHAT_DOWNLOAD_IMAGES", "label": "下载图片", "type": "switch", "help": "是否下载文章中的图片资源"},
            {"key": "WECHAT_ALLOWED_IMAGE_FORMATS", "label": "允许图片格式", "type": "text", "help": "逗号分隔，如 jpg,jpeg,png"},
            {"key": "WECHAT_IMAGE_DOWNLOAD_CONCURRENCY", "label": "图片下载并发", "type": "number", "help": "图片下载并发数"},
            {"key": "WECHAT_IMAGE_SAVE_DIR", "label": "图片保存目录", "type": "text", "help": "相对于 data/ 的路径"},
            {"key": "WECHAT_CONTENT_SAVE_DIR", "label": "文章保存目录", "type": "text", "help": "相对于 data/ 的路径"},
        ],
    },
    {
